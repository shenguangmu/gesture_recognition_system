...
| **摄像头出图** | ❌ **仍未通** —— 帧计数 0，见 §六 |
| **PL 算法 bug** | ✅ **发现 `crop_scale` 整数块平均缺陷**（**2026-09-23 已修复**，见 §十 + 09-23 日志） |
| 蓝屏（2 次） | ⚠ 未解决，见 §七 |
...
## 十、⭐ 发现的 PL 算法 bug：`crop_scale` 整数块平均缺陷（**2026-09-23 已修复**）
...
> ✅ **本节的两个缺陷已于 2026-09-23 修复并验证** ——
> 见 [`board-test-log-2026-09-23.md`](board-test-log-2026-09-23.md)。
> 修好后板级输出与 golden **逐字节一致**（三次不同输入 / 三种 ROI，各 0/9216）。
> **本文保留当时的现场记录，不要拿本文的 ❌ 结论当现状。**
