...
> 全划分会展开成 96 组独立寄存器（并配一个 96 选 1 译码器）。
>
> 不加划分则退化为单块 RAM、产生访存依赖 → **II=2、周期翻倍**
> （实测 4,308,939 → 2,236,419，**快 1.93×**）。
>
> **这是有意的取舍**：宁可吃 LUT 也要 II=1。回退方式见
> `src_hls/gesture_preproc.cpp` 里 `acc[]` 的注释。
>
> ⚠ **BRAM (18.21%) / DSP (27.73%) / IOB (28.00%) 三项不变。**
...
>
> ✅ **2026-09-23 复查仍成立**：最新一次实现 WNS = **+0.199 ns**，
> 最差路径依然落在 `v_tc` 内部。说明本设计逻辑**没有吃掉这段余量**
> （该次实现同时把 `crop_scale` 改成按比例分配、LUT 涨到 45.94%，
> 占用大幅上升而 WNS 只从 +0.265 变到 +0.199 —— 进一步印证
> 瓶颈不在本设计逻辑上）。
>
