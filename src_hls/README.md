...
| `tb_gesture.cpp` | csim 测试台（5 组用例，逐位比对） |
...
| **板级输出 vs golden** | ❌ **对不上** —— 2026-09-22 喂真实图像后发现，见下 |
| Python golden | ✅ 自检全过 |

> ## ⚠⚠ 已知 bug：`crop_scale` 整数块平均**填不满 96**
>
> **2026-09-22 喂了第一张真实图像后暴露。明天修。**
>
> ```
> step_x = (roi_w + 96 - 1)/96 = (320+95)/96 = 4
...
> **实测非零 2681 ≈ 理论 2816** ✓
>
> **为什么 csim 没抓到**：csim 用**同一套 C++ golden**，
> 若它也没覆盖 `roi_w` 整除边界，两边**一起错**（pitfalls **P6** 同类）。
> 而 Python golden 走**浮点比例缩放**，与整数块平均语义本就不同，
> 直到上板对拍才显形。
>
> **修法**：改"按比例分配"（第 j 个输出覆盖 `[j*roi_w/96, (j+1)*roi_w/96)`），
> 保证恰好 96 个输出。**不能改 golden 去迁就** ——
> 接口契约是"96×96 全填充"，迁就等于让参考实现跟着错。
>
> 详见 `docs/board-test-log-2026-09-22.md` §十、`skill/pitfalls/README.md` **P14**。
