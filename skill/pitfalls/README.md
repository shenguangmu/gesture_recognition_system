...
| **整数块平均填不满输出 → 图像只填了 80×80 却当作 96×96** | **P14** |
...
## P14 · 整数块平均**填不满输出** —— 只填了 80×80 却当作 96×96

> **来源**：2026-09-22。**喂了第一张真实图像之后**才暴露的 HLS 实现缺陷。
> 与 P6 同族：**"仿真通过"掩盖了错误**，只是这次的掩盖方式是
> "合成测试图恰好不触发"。

**现象**

链路完全跑通（单帧 0.005 s、输出 96×96、动态范围 0~255），
**但与 golden 参考对不上**：

| 输入 | 实现输出非零 | golden 非零 | 比值 |
|---|---|---|---|
| 真实照片 | 3849 | 2414 | 1.6× |
...
```c
step_x = (roi_w + OW - 1) / OW;    // (320 + 96 - 1) / 96 = 4
...
for (int x = 0; x < width; x++) {
    if (in_roi) {
        cx++;
        if (cx == step_x) { ... ox[n_out++] = acc / (step_x*step_y); }
    }
}
while (n_out < G::OPIX) ox[n_out++] = 0;   // ← 补零兜底
```

**`roi_w = 320` 能被 `step_x = 4` 整除** →

```
实际输出 = 320 / 4 = 80 个/行     但 OW = 96，需要 96 个
→ 只填了 80 × 80 = 6400，剩 2816 个是补零
```

**实测 2681 ≈ 理论 2816** ✓ 吻合。逐行非零大多正好 **94**（= 96−2），
也是"前 80 个有内容、后面补零"的表现。

**为什么一直没暴露**

1. **合成测试图恰好不触发** —— 此前验证**从没喂过真实图像**，
   而 `fill_test_pattern()` 的图案正好没踩到边界
2. **csim 用"同一套 C++ golden"** —— 若它也没覆盖整除边界，
   两边**一起错**（与 **P6** 同一类）
3. **Python golden 走浮点比例缩放**，与整数块平均**语义本就不同**，
   直到对拍才显形

**⭐ 定位方法：用"答案已知的合成图"代替真实照片**

真实照片结构复杂，差异图看不出规律。换成**黑底 + 白块**后：
...
```c
// 第 j 个输出覆盖源区间 [j*roi_w/OW, (j+1)*roi_w/OW)
// → 无论 roi_w 是多少，都恰好产生 OW 个输出
```

⚠ **不能改 golden 去迁就实现** —— 接口契约是"96×96 全填充"，
迁就等于**让参考实现跟着错**。

> **一般化**：
> 1. **合成图是定位利器** —— 真实数据验证正确性，
>    **合成数据定位"错在哪一级"**。答案已知，差异才解读得了。
> 2. **"跑通"≠"正确"** —— 本例链路全通、耗时正常、输出非空，
>    **看起来完全正常**，但结果是错的。
> 3. **整数除法做缩放的经典陷阱** —— 凡 `(n + k - 1) / k` 的块划分，
>    都要问一句：**n 能被 k 整除时会怎样？**
...
本目录只列了**"工具流程全过、只有上板才暴露"**这一类中最典型的 **14 条**
（P9/P10 是 2026-09-21 补的，P11~P14 是 2026-09-22 补的）。
