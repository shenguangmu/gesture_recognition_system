...
| **BD 脚本里设的 IP 参数"没生效"，只给 WARNING** | **P11** |
| **引脚映射靠"推理"而非实物丝印，14 个信号全错位** | **P12** |
| **引脚位置与 FPGA 时钟专用脚不相交 → 布线直接失败** | **P13** |
...
## P11 · BD 里设的 IP 参数**没生效**，只给 WARNING —— XCLK 因此不出

> **来源**：2026-09-22 摄像头不出图的最终根因。**与 P10 完全同族**：
> 参数没进去、不报错、只在硬件上表现为怪现象。

**现象**

摄像头通路全死：XCLK 无输出 → SCCB 无 ACK（`cfg_error=1`）→ 不出图
→ VDMA 帧计数恒 1。**FPGA 侧没有任何报错。**

**根因**

`bd_video.tcl` 里给 Clocking Wizard 设 MMCM 参数想避开 VCO 上限：
...
**`OVERRIDE_MMCM` 这个开关在常规配置界面里不明显** —— 最后是靠打印
**完整属性表**才找到的：
...
> `C_SG_LENGTH_WIDTH`（P10）、`C_PROBE{N}_WIDTH`、以及本条。
> 它们的共同点是**只给 WARNING**，而 WARNING 在几千行构建日志里
> 根本不会有人看。

---

## P12 · 引脚映射靠"推理"而非**实物丝印** —— 14 个信号全错位

> **来源**：2026-09-22。摄像头不出图的**真正根因** ——
> 排查耗时一个月，而且中途多次被"推理看似自洽"误导。

**现象**

摄像头完全不动，**但所有"推理出来的证据"都自洽**：

```
从 Y18 输出 XCLK → ILA 抓该引脚，有 24 MHz 方波 ✓（FPGA 确实在工作）
io_pclk 恒 1     → 摄像头没吐时钟
cfg_error = 1    → SCCB 没收到 ACK
```

看起来像"模块坏了"或"SCCB 时序问题"，**唯独不像"接错了"** ——
因为"XCLK 明明有输出"这一点强烈暗示 FPGA 侧是好的。

**根因**

`video_io.xdc` 的映射是从**模块原理图的画法***推理*出来的：
原理图 J2 用"左列 12→7、右列 1→6"的非标准画法，于是选了"镜像解读"。
**这个推理是错的。**

**决定性证据来自实物丝印**（读连接器上印的信号名）：
...
**`5/6 = GND/3V3`、`11/12 = GND/3V3` —— 正是 Pmod 规范的标准位置。**
...
- XCLK 输出到 Y18 → **模块 pin1 = NC** → 摄像头永远没主时钟
- `io_pclk` 从 U18 读 → **pin7 = NC** → 读悬空脚 → **恒 1**

**⭐ 交叉验证**：正确映射下**未使用的 Y18/U18**，恰好对应模块
...
1. **推理链自洽**：图片原理图 → 镜像解读 → 选 ja[] → 得引脚，
   每一步都能自圆其说，**没有任何一步会报错**
2. **"XCLK 有输出"给了强烈的反向暗示**：既然 FPGA 在发时钟，
   直觉上就不会怀疑"接错线"
3. **"镜像假设"当初还配了个看似有力的理由**（"只有它能让电源脚对齐"）
   —— 而**实测证明标准编号下电源脚本来就对齐**
4. 文档写着"必须万用表复核"，**而那一步始终没做** —— 已知风险拖了一个月

**对策**

1. **优先读实物丝印**。丝印是连接器上的直接标注，**不需要任何推理**；
   原理图可能有意象化的画法，丝印不会。
2. **用电源脚当锚点**：Pmod 规范里 **5/6/11/12 = GND/VCC/GND/VCC**。
   数一数丝印的电源脚落在哪，立刻能判断是标准还是镜像编号。
3. **算完做自洽检查**：正确映射下**空脚应正好对应 NC 脚**。对不上就是推错了。
4. ⚠ **别让"某条推论看起来有道理"替代实测** ——
   "只有镜像能让电源对齐"就是个**听起来很对、实际错得很彻底**的论证。

> **一般化**：**推理链自洽 ≠ 推理正确。**
> 从图片/文档"推导"物理连接时，每一步都可能引入假设，
> 而**假设错了不会报错，只会让所有后续结论一起错**。
> 凡是能直接看实物（丝印、通断）的，**就别推理**。

---

## P13 · 引脚位置与 FPGA **时钟专用脚不相交** → 布线直接失败

> **来源**：2026-09-22。修好 P12（引脚映射）之后**才暴露出来**的独立问题 ——
> 上一轮**综合能过、实现直接失败**。
...
摄像头 PCLK 在模块的 **pin 2**，按标准 Pmod 编号落在 **ja[1] = Y19**。
但 FPGA 上能驱动 BUFG 的 **CCIO（时钟专用）脚**，Pmod A 上**只有 U18/U19**：

| ja | 引脚 | 是否 CCIO | 模块上是什么 |
|---|---|---|---|
| ja[1] | **Y19** | ❌ 普通 IO | **PCLK** ← 需要 CCIO |
| ja[4] | U18 | ✅ CCIO | pin 7 = **NC** ← 空脚 |
| ja[5] | **U19** | ✅ CCIO | pin 8 = **XCLK**（输出，用不上 CCIO）|

**PCLK 的物理位置与 FPGA 的时钟脚完全不相交** ——
两个 CCIO 一个接空脚、一个接输出，唯独需要时钟脚的 PCLK
落在了普通 IO 上。**这是模块引脚排布决定的，无法用改约束之外的办法解决。**

**⭐ 顺带解释了"为什么原来能用"**

旧的错误映射把 PCLK 放在 **U18（正好是 CCIO）** ——
所以**布线一直是通过的**。也就是说：
...
用 Vivado 自己给的 override（报错信息里直接写了）：

```tcl
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets io_pclk_IBUF]
```

**为什么这个 override 可以接受**（⚠ 不能因为"报错让这么干"就无脑用）：

| 项 | 值 |
|---|---|
| PCLK 频率 | **24 MHz**（周期 41.7 ns） |
| 输入延时约束窗口 | **1.5 ns** |
| 非专用路径额外插入延时 | 典型几 ns |

**几十倍余量**，插入延时不足以吃掉采样窗口。**但必须靠实现后时序报告确认**，
不能想当然 —— 判据是 `cam_pclk` 组 WNS 为正。

**⚠ 这是本项目唯一一处"明知不推荐仍然采用"的 override**，
理由是**硬件引脚排布的物理限制**，不是图省事。
换板卡/换模块时这条要重新评估。

> **一般化**：**"改对了约束"可能引出"之前被掩盖的问题"。**
> 修完一个 bug 要**重跑到底**（综合 → 实现 → 比特流），
> 而不是只跑到综合通过就算完 —— 本条的失败**恰恰出在实现阶段**。
> 另外：**时钟输入必须落 CCIO 脚**是 FPGA 的硬性物理要求，
> 选板卡/选模块时要**先把时钟脚和 CCIO 对齐**再定接线方案。

---

本目录只列了**"工具流程全过、只有上板才暴露"**这一类中最典型的 **13 条**
（P9/P10 是 2026-09-21 补的，P11/P12/P13 是 2026-09-22 补的）。
更完整的 23 条（HLS 接口/pragma、Vitis 命令行、BD/Tcl、Zynq 协同、环境噪声）
见项目仓库 `vivado/README.md` 的「踩过的 23 个坑」一节，
**第 20/21/22/23 条分别对应本目录的 P10 / P11 / P12 / P13**。
