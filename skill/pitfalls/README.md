...
本目录只列了**"工具流程全过、只有上板才暴露"**这一类中最典型的 **10 条**
（P9 / P10 是 2026-09-21 上板实测后补的）。
更完整的 20 条（HLS 接口/pragma、Vitis 命令行、BD/Tcl、Zynq 协同、环境噪声）
见项目仓库 `vivado/README.md` 的「踩过的 20 个坑」一节，
其中前 15 条按 A–E 五类组织，**第 20 条 = 本目录的 P10**（同一件事的两处记录）。
