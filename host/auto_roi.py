...
================================================================================
  附录 C：ROI 机制到底管什么用（2026-09-26 实测）
================================================================================

  ⚠⚠ **先更正一条本文件外部的错误结论。**

  有一次分析得出过「ROI 跟随让位置噪声从 19.7% 涨到 31.1%，跟随是坏的」。
  **那个结论是错的，是测试方法造的假象。**

  错在哪：用 `np.roll` 式平移**整幅图**，空出来的边是**纯黑（Y=0）**。
  而前景判据是 `|Y - bg| > 25` —— 黑边自己就满足 `|0-32| = 32 > 25`，
  于是**黑边被判成前景**，bbox 铺满整个画面，中心当然乱跑。

  ⇒ 教训：测"手在动"时，**背景必须不动**。平移整幅图会把背景一起动掉，
    还顺手在边上引入一大块假前景。正确做法是**用手部 mask 把
    手抠出来、贴到同一张背景板的不同位置**。

  ────────────────────────────────────────────────────────────────
  ① ROI 跟随对**平移**几乎完美（这是它的设计目的）
  ────────────────────────────────────────────────────────────────

  方法：手部 mask 抠图 → 贴到同一背景的不同位置 → 看输出。

      手位移 dx     bg      fg%     ROI x    Δroi_x（理想 = dx）
      ──────────────────────────────────────────────────────
        -80       208.0   22.8       12         -80   ✅
        -40       208.0   22.8       52         -40   ✅
         +0       208.0   22.8       92          +0   ✅
        +40       208.0   22.8      132         +40   ✅
        +80       208.0   22.8      172         +80   ✅

  **严格 1:1，bg 与 fg% 完全不变。** 合成图与实拍图都是这个结果。

  对输出的影响：**平移 ±40 px → 输出变化 ~0%**。
  （对比：固定 448 ROI 时同样平移会变 20%+）

  ② 所以 ROI 的真实价值排序

      作用                    状态
      ──────────────────────────────────────────────
      ① 分辨率分配（放大手）    ✅ 有效，且是当前的主要价值
      ② 位置归一化              ✅ 有效（1:1 跟随，实测确认）
      ③ 尺度归一化              ❌ **没做** —— 这是下一步的改进点
  ...
  类内波动（ROI 跟随，手/背景分离测试）：

      平移 x ±40        ~0%        ← 被跟完完全抵消
      平移 y ±20       9.6~11.3%
      **缩放 ±10%      14.8~17.3%**  ← 主要残余
      亮度 ±20         < 11%

  原因：**ROI 边长固定 448**，手离镜头近一点远一点，输出里也就大一点小一点。

  ────────────────────────────────────────────────────────────────
  ④ ⭐ 自适应尺寸能把尺度噪声砍掉一半（已实测，但**未上板**）
  ────────────────────────────────────────────────────────────────

  做法：ROI 边长 ∝ 手 bbox 最大边（取整到 32 的倍数）。

      缩放      A 固定 448      B 自适应尺寸
      ─────────────────────────────────────────
      x0.85       21.4%          4.2%   (ROI 384)
      x0.90       16.8%          6.4%   (ROI 416)
      x1.10       17.3%          9.9%   (ROI 480)
      x1.15       23.7%         14.8%   (ROI 480)

  ⭐ **这条推翻了本文件「为什么默认 448」里给"尺寸恒定"的主要理由。**

    那是「尺寸恒定 → 缩放比恒定 → `gain=256` 标定一直有效」。
    但实测：**二值模式下 gain 从 128 扫到 512（4 倍），输出只差 0~4.5%**
    —— 因为 `thresh_mode=1` 下前景像素本来就全部饱和在 255。

    ⇒ 自适应尺寸的**真实代价**是「手在输出里的表观尺寸会逐帧变，
      CNN 需要做尺寸增强」——**不是** gain。
      （`gain` 是 Q8 增益，补的是"在 96x96 上算 Sobel 梯度偏弱"，
       见 `src_hls/gesture_preproc.cpp:15`。）

  ⚠ ×1.05 以上优势变小 —— 因为 480 是画面高度上限，ROI 撑满了没法再跟着放大。
    所以自适应若要上，**必须同时要求构图让手别占满画面**（见文件头
    「贴边跟不动」里的构图对照表）。
  ...
  ────────────────────────────────────────────────────────────────
    ...
    · 只用 **2 张手势图**（合成图 + 力手实拍）算的 —— 是**量级估计**，不是精确结论
    · 缩放手是**用 LANCZOS 合成**的，不是真实远近变化（真实变化还带透视和光照变化）
    · 全部走 **PC 侧 golden**（与硬件逐位一致，但**不是硬件**）
    · **未上板**。真数据集到位后，用同样的方法（mask 抠图 + 贴回背景）重测。

    **别拿这节的数字去定类别数上限** —— 那只在真数据集上测才有意义。
    本节能确定的是三件事：
      1. ROI 跟随对平移是**有效**的（之前那条"跟随坏了"是错的）
      2. 残余噪声主要在尺度
      3. 自适应尺寸有明确的改进空间，且代价不是 gain
