...
================================================================================
  语义：方案 B（尺寸恒定 320x320，中心跟手）
================================================================================

  ┌──────────┐  算出前景包围盒 bbox
  │  手 bbox  │  bbox 中心 = (cx, cy)
  └──────────┘  ROI 中心 = bbox 中心，**尺寸恒为 320x320**
                ROI 左上角 = (cx-160, cy-160)，再钳到 [0, 640-320] x [0, 480-320]

  **为什么尺寸恒定，而不是"bbox + margin 外扩"（方案 A）**：
    · 尺寸恒定 → `crop_scale` 的缩放比恒定（320/96）→
      Sobel 的 `gain=256` 标定**一直有效**。
      方案 A 手一张一缩，96x96 里的手忽大忽小，CNN 更难学，gain 也要跟着改。
...
================================================================================
  ...
  **手会重新掉出框外**，只是程度比固定 ROI 轻。
  ...
  缓解手段（择一，都需要先上板实测再定）：
    · 要求用户在构图时把手放在画面中部（最省事，写进操作说明）
    · 手靠边时**动态缩小 ROI**（尺寸不再恒定 → gain 标定要跟着改，CNN 也要重训）
    · 上 PIL：把"检测到的中心"当目标，但 ROI 用**平滑跟随**而非直接跳变
...
def main() -> int:
    ...
    if not res['fits'] and res['bbox'] is not None:
        ...
        print("  [!!] ⚠ ROI 装不下前景！左右上下各切掉 %d/%d/%d/%d 像素"
              % (c[0], c[1], c[2], c[3]))
        print("       → 调大 --size（当前 %d）。手高/宽 = %d/%d"
              % (args.size, res['bbox'][3] - res['bbox'][1] + 1,
