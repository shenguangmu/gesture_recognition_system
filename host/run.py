...
# 默认 ROI —— 屏幕居中的 448x448（两边各留 (640-448)/2=96、(480-448)/2=16）
#
# ⚠ **尺寸**用 448 而不是 320。2026-09-24 `auto_roi.py` 实测：固定 320
#   会把「合成手势图」的上下各切掉约 35 px（那张的手本身就 391 高 > 320）。
#
# ⚠ **位置**是"屏幕居中"，不是"某个具体手的位置"。力手那张 auto_roi 给出的是
#   (96,32)，比这里低 16 px —— 那是针对那张图的，不是通用默认。
#   默认值的语义是"不知道手在哪时先居中"。
#
# ⚠ 448 的代价：**纵向活动余量只剩 480-448 = 32 px**，手稍靠上/靠下就被钳在
#   边界上。所以：
#     · 手在画面中部 → 回车用这个默认
#     · 手靠边 / 想自动跟手 → 先跑 auto_roi.py，把它给的那行填进来
#
# ⚠ 本文件的默认与 `gesture_overlay.config()` / `bringup_check.py` 的 320
#   **故意不一致**：那两个是**数据通路自检**，只验"数据流不流得动"、
#   不看手，320 是它们的已验证值。别去"统一"它们。
ROI = dict(x=96, y=16, w=448, h=448)
