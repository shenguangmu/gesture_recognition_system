...
> ## ⭐ 刚拿到这份仓库？先看 **[`HANDOFF.md`](HANDOFF.md)**
>
> 一页回答「从哪开始」：不需要 license 就能跑的三条验证命令、
> 三条按目的分的路线（跑比特流 / 调 CNN / 改代码）、
> 以及比特流藏在 `xsa` 里要怎么取。
>
...
> 详见下文「原 Sobel 工程在哪」。
>
> ## 当前进度（2026-09-23；**已上板，静态图通路与 golden 逐字节一致**）
>
> | 部分 | 内容 | 状态 |
> |---|---|---|
...
> | **PS 侧驱动** | `sw/preproc_driver.c` | ✅ 主机自检 **26/26** |
> | **时序 / 比特流 / XSA** | 实现后实测 | ✅ **WNS +0.199 / WHS +0.051 ns**（2026-09-23，`use_ila=0`） |
> | **板级实测 ②③** | DDR 自检 + 只跑预处理链 | ✅✅ **11/11 全过**（2026-09-21） |
> | **静态图喂入** | 真实图片 → DDR → 全流水线（**方案 A，不需要摄像头**） | ✅ **充分验证**：单帧 0.006 s、输出 96×96，**与 golden 逐字节一致** |
> | **输出 vs golden** | 与 Python 参考对拍 | ✅✅ **逐字节一致** —— 三次不同输入 / 三种 ROI，各 **0/9216 不一致**（2026-09-23） |
| **给 PS/CNN 侧的样例数据** | `samples/` | ✅ **已备好**：输入 `.bin` + **硬件实测对过的** golden，PC 上即可调模型 |
| **给 PS/CNN 侧的交付说明** | `docs/pl-to-ps-handoff.md` | ✅ **已备好**：数据格式 + 批量造数据 + 待拍板项 + 联调接口 |
> | **板级实测 ④~⑧** | 摄像头通路 | ⚠ **未通**，但**已不是关键路径**（方案 A 可满足赛题硬指标） |
> | HDMI 输出 | TMDS 编码器 | ⚠️ 未做（**上板不要接 HDMI 线**） |
>
> ⚠⚠ **上板前必读**（两份实测记录，按时间倒序）：
> - **`docs/board-test-log-2026-09-23.md`**（最新）——
>   修掉两个真缺陷：`crop_scale` 固定步长、**跳过 ROI 外行不读流导致
>   `roi_y` 完全失效**；板级输出首次与 golden 逐字节一致
> - `docs/board-test-log-2026-09-22.md` ——
>   MMCM 参数静默失效、引脚映射全错、CCIO 冲突、蓝屏
> - `docs/board-test-log-2026-09-21.md` ——
>   **DMA `C_SG_LENGTH_WIDTH` 默认 14 位**（修复前的比特流上板必坏）
> 
> ⚠ **WNS 逐次波动较大**：同一设计多次实现的实测为
> +0.873 / +1.177 / +0.265 / **+0.199** ns（后者是 2026-09-23 最新一次）。
> 布线是随机过程，每次结果不同。
>
...
> 三次不同输入 / 三种 ROI）。摄像头通路仍未通（卡在 SCCB，`cfg_error=1`），
> 但它**已不是关键路径** —— 方案 A 满足赛题硬指标。
> ⚠ 上板前必读 `docs/hardware-checklist.md` §3.3（引脚万用表复核）。
> ⚠⚠ **别用 ILA 调试**：`use_ila=1` 会让 BRAM 涨到 52.5%，并引入
> `XCLK→PS时钟` 的 hold 违例（WHS −1.830 ns，跨异步域、物理修不了）。
>
> ### 一键回归
>
